...
PAGE_PATH = os.path.join(ROOT, "pages/68_tax_loss_harvesting_assistant.py")
...
def test_required_patterns():
    """Tax Loss Harvesting Assistant page follows peachstatesavings.com coding standards."""
    src = _source()
...
def test_no_hardcoded_secrets():
    """Tax Loss Harvesting Assistant page must not contain hardcoded API keys or passwords."""
    src = _source()
...
def test_feature_metadata():
    """Tax Loss Harvesting Assistant — basic file sanity."""
    assert "tax_loss_harvesting_assistant" != ""
