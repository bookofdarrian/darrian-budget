...
PAGE_PATH = os.path.join(ROOT, "pages/58_loan_amortization_calculator.py")
...
def test_required_patterns():
    """Loan Amortization Calculator page follows peachstatesavings.com coding standards."""
    src = _source()
...
def test_no_hardcoded_secrets():
    """Loan Amortization Calculator page must not contain hardcoded API keys or passwords."""
    src = _source()
...
def test_feature_metadata():
    """Loan Amortization Calculator — basic file sanity."""
    assert "loan_amortization_calculator" != ""
