...
PAGE_PATH = os.path.join(ROOT, "pages/83_rsu_vest_calendar_tax_optimizer.py")
...
def test_required_patterns():
    """RSU Vest Calendar + Tax Withholding Optimizer page follows peachstatesavings.com coding standards."""
    src = _source()
...
def test_no_hardcoded_secrets():
    """RSU Vest Calendar + Tax Withholding Optimizer page must not contain hardcoded API keys or passwords."""
    src = _source()
...
def test_feature_metadata():
    """RSU Vest Calendar + Tax Withholding Optimizer — basic file sanity."""
    assert "rsu_vest_calendar_tax_optimizer" != ""
