...
PAGE_PATH = os.path.join(ROOT, "pages/77_investment_rebalancer.py")
...
def test_required_patterns():
    """Investment Portfolio Rebalancer page follows peachstatesavings.com coding standards."""
    src = _source()
...
def test_no_hardcoded_secrets():
    """Investment Portfolio Rebalancer page must not contain hardcoded API keys or passwords."""
    src = _source()
...
def test_feature_metadata():
    """Investment Portfolio Rebalancer — basic file sanity."""
    assert "investment_rebalancer" != ""
