...
PAGE_PATH = os.path.join(ROOT, "pages/33_hsa_receipt_categorizer.py")
...
def test_required_patterns():
    """HSA Receipt Auto-Categorizer page follows peachstatesavings.com coding standards."""
    src = _source()
...
def test_no_hardcoded_secrets():
    """HSA page must not contain hardcoded API keys or passwords."""
    src = _source()
    for token in ["sk-ant-", "password=", "Bearer ", "hardcoded"]:
        assert token not in src, f"Hardcoded secret found: {token!r}"


def test_feature_metadata():
    """HSA Receipt Auto-Categorizer — basic file sanity."""
    assert "hsa_receipt_categorizer" != ""
