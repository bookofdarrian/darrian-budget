...
PAGE_PATH = os.path.join(ROOT, "pages/97_cc_admissions_decision_tracker.py")
...
def test_required_patterns():
    """CC: Admissions Decision Tracker page follows peachstatesavings.com coding standards."""
    src = _source()
...
def test_no_hardcoded_secrets():
    """CC: Admissions Decision Tracker page must not contain hardcoded API keys or passwords."""
    src = _source()
...
def test_feature_metadata():
    """CC: Admissions Decision Tracker — basic file sanity."""
    assert "cc_admissions_decision_tracker" != ""
